...
let string_to_p_StringList ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct (Json_encoding.list Json_encoding.string)) in
    _string_to ~p ~kind:(`Array [(`List (`String))])
...
let string_of_t_ab8d71cc96 ~p ~op ~loc ~style ~explode (_x : t_ab8d71cc96) =
  _string_of ~kind:(
    begin match _x with
    | T_6aa55531fa _x ->
      `ObjectN
...
let namevalues_to_p_Int ~p ~loc ~style ~explode (_x : (string*string) list) =
  let dtr = (Json_encoding.destruct Json_encoding.int) in _namevalues_to ~p
    ~kind:(`Integer) ~dtr ~loc ~style ~explode _x

let string_of_t_4e604540e7 ~p ~op ~loc ~style ~explode (_x : t_4e604540e7) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_4e604540e7)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_4e604540e7 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_4e604540e7) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_8dfdeac1ad ~p ~op ~loc ~style ~explode (_x : t_8dfdeac1ad) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("director", begin match _x.director with | None -> `Null | Some _x ->
        `Boolean end);
       ("executive", begin match _x.executive with | None -> `Null
        | Some _x -> `Boolean end);
       ("legal_guardian", begin match _x.legal_guardian with | None -> `Null
        | Some _x -> `Boolean end);
       ("owner", begin match _x.owner with | None -> `Null | Some _x ->
        `Boolean end);
       ("representative", begin match _x.representative with | None -> `Null
        | Some _x -> `Boolean end)])
    ~ctr:(Json_encoding.construct Encoders'.t_8dfdeac1ad)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_8dfdeac1ad ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_8dfdeac1ad) in _string_to ~p
    ~kind:(`ObjectN
...
let string_of_t_52a890434c ~p ~op ~loc ~style ~explode (_x : t_52a890434c) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("director", begin match _x.director with | None -> `Null | Some _x ->
...
let string_to_t_52a890434c ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_52a890434c) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any); ("director", `Boolean); ("executive", `Boolean);
              ("legal_guardian", `Boolean); ("owner", `Boolean);
              ("representative", `Boolean)]) ~dtr ~loc ~style ~explode _x
...
let string_of_t_9d68bdd713 ~p ~op ~loc ~style ~explode (_x : t_9d68bdd713) =
  _string_of ~kind:(
    begin match _x with
    | T_6705dd5948 _x ->
      `ObjectN
...
let string_of_t_4a4b8daa1f ~p ~op ~loc ~style ~explode (_x : t_4a4b8daa1f) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any); ("type", let _x = _x.type_ in `String);
       ("user", begin match _x.user with | None -> `Null | Some _x -> `String
        end)]) ~ctr:(Json_encoding.construct Encoders'.t_4a4b8daa1f)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_4a4b8daa1f ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_4a4b8daa1f) in _string_to ~p
...
let string_of_t_4ce91395e3 ~p ~op ~loc ~style ~explode (_x : t_4ce91395e3) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any); ("type", let _x = _x.type_ in `String);
       ("user", begin match _x.user with | None -> `Null | Some _x -> `String
        end)]) ~ctr:(Json_encoding.construct Encoders'.t_4ce91395e3)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_4ce91395e3 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_4ce91395e3) in _string_to ~p
    ~kind:(`ObjectN [("", `Any); ("type", `String); ("user", `String)])
    ~dtr ~loc ~style ~explode _x
...
let string_of_t_01b97bcd1e ~p ~op ~loc ~style ~explode (_x : t_01b97bcd1e) =
  _string_of ~kind:(
    begin match _x with
    | T_51dd3dd546 _x ->
      `ObjectN
...
let string_of_t_e84cc44f8f ~p ~op ~loc ~style ~explode (_x : t_e84cc44f8f) =
  _string_of ~kind:(
    begin match _x with
    | T_b5ed09371b _x ->
      `ObjectN
...
let string_of_t_b478178155 ~p ~op ~loc ~style ~explode (_x : t_b478178155) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_b478178155)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_b478178155 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_b478178155) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_2ef034b1cb ~p ~op ~loc ~style ~explode (_x : t_2ef034b1cb) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_2ef034b1cb)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_2ef034b1cb ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_2ef034b1cb) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let namevalues_to_p_Bool ~p ~loc ~style ~explode
  (_x : (string*string) list) =
  let dtr = (Json_encoding.destruct Json_encoding.bool) in _namevalues_to ~p
    ~kind:(`Boolean) ~dtr ~loc ~style ~explode _x

let string_of_t_9b9d8d63ef ~p ~op ~loc ~style ~explode (_x : t_9b9d8d63ef) =
  _string_of ~kind:(
    begin match _x with
    | T_143678df45 _x ->
      `ObjectN
...
let string_of_t_0b0b7bdb7e ~p ~op ~loc ~style ~explode (_x : t_0b0b7bdb7e) =
  _string_of ~kind:(
    begin match _x with
    | T_7da3863d89 _x ->
      `ObjectN
...
let string_of_t_12bf81c281 ~p ~op ~loc ~style ~explode (_x : t_12bf81c281) =
  _string_of ~kind:(
...
let string_to_t_12bf81c281 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_12bf81c281) in _string_to ~p
...
let string_of_t_976c399de7 ~p ~op ~loc ~style ~explode (_x : t_976c399de7) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_976c399de7)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_976c399de7 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_976c399de7) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_8efa015a15 ~p ~op ~loc ~style ~explode (_x : t_8efa015a15) =
  _string_of ~kind:(
    begin match _x with
    | T_13e89791f1 _x ->
      `ObjectN
...
let string_of_t_d1452a2e6d ~p ~op ~loc ~style ~explode (_x : t_d1452a2e6d) =
  _string_of ~kind:(
    begin match _x with
    | T_753b9f6893 _x ->
      `ObjectN
...
let string_of_t_c4284eadfd ~p ~op ~loc ~style ~explode (_x : t_c4284eadfd) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
...
let string_to_t_c4284eadfd ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_c4284eadfd) in _string_to ~p
...
let string_of_t_c086813c51 ~p ~op ~loc ~style ~explode (_x : t_c086813c51) =
  _string_of ~kind:(
    `Array
            ...
            (`ObjectN
               [("", `Any);
                ...
                ("tax_rates", begin match _x.tax_rates with | None -> `Null
                 ...
                 | StringList _x ->
                   `Array
                     ((List.map (fun (_x : string) -> `Singleton (`Null)) _x))
...
let string_to_t_c086813c51 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_c086813c51) in _string_to ~p
    ~kind:(`Array
             [(`List
                 (`ObjectN
                    [("", `Any); ("amount", `Integer);
                     ("description", `String);
                     ("invoice_line_item", `String); ("quantity", `Integer);
                     ("tax_amounts",
                      `Choice
...
let string_of_t_fa0c590277 ~p ~op ~loc ~style ~explode (_x : t_fa0c590277) =
...
let string_to_t_fa0c590277 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_fa0c590277) in _string_to ~p
...
let string_of_t_935a453a3f ~p ~op ~loc ~style ~explode (_x : t_935a453a3f) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_935a453a3f)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_935a453a3f ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_935a453a3f) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_5780adc875 ~p ~op ~loc ~style ~explode (_x : t_5780adc875) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_5780adc875)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_5780adc875 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_5780adc875) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_7d4b84944c ~p ~op ~loc ~style ~explode (_x : t_7d4b84944c) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("shipping_rate", begin match _x.shipping_rate with | None -> `Null
...
let string_to_t_7d4b84944c ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_7d4b84944c) in _string_to ~p
    ~kind:(`ObjectN [("", `Any); ("shipping_rate", `String)])
    ~dtr ~loc ~style ~explode _x
...
let string_of_t_3acf43dc3b ~p ~op ~loc ~style ~explode (_x : t_3acf43dc3b) =
  _string_of ~kind:(
    `Array
...
let string_to_t_3acf43dc3b ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_3acf43dc3b) in _string_to ~p
    ~kind:(`Array
             [(`List
                 (`ObjectN
                    [("", `Any); ("amount", `Integer);
...
let string_of_t_2551b208c1 ~p ~op ~loc ~style ~explode (_x : t_2551b208c1) =
  _string_of ~kind:( `ObjectN [("", `String)])
    ~ctr:(Json_encoding.construct Encoders'.t_2551b208c1)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_2551b208c1 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_2551b208c1) in _string_to ~p
    ~kind:(`ObjectN [("", `String)]) ~dtr ~loc ~style ~explode _x
...
let string_of_t_8cd3871a8a ~p ~op ~loc ~style ~explode (_x : t_8cd3871a8a) =
  _string_of ~kind:(
    begin match _x with
    | T_29c5820fae _x ->
      `ObjectN
...
let string_of_t_4f49500d45 ~p ~op ~loc ~style ~explode (_x : t_4f49500d45) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_4f49500d45)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_4f49500d45 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_4f49500d45) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_e03d9a444b ~p ~op ~loc ~style ~explode (_x : t_e03d9a444b) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_e03d9a444b)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_e03d9a444b ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_e03d9a444b) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_d8f80ab533 ~p ~op ~loc ~style ~explode (_x : t_d8f80ab533) =
  _string_of ~kind:(
    begin match _x with
    | T_05166771dc _x ->
      `ObjectN
...
let string_of_t_194d7f2624 ~p ~op ~loc ~style ~explode (_x : t_194d7f2624) =
  _string_of ~kind:(
    begin match _x with
    | T_4a62a84b9e _x ->
      `ObjectN
...
let string_of_t_76bf2704bf ~p ~op ~loc ~style ~explode (_x : t_76bf2704bf) =
  _string_of ~kind:(
    begin match _x with
    | T_019e891337 _x ->
      `ObjectN
...
let string_of_t_2657bcad54 ~p ~op ~loc ~style ~explode (_x : t_2657bcad54) =
  _string_of ~kind:(
    begin match _x with
    | T_872f5780fe _x ->
      `ObjectN
...
let string_of_t_512e4129fd ~p ~op ~loc ~style ~explode (_x : t_512e4129fd) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_512e4129fd)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_512e4129fd ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_512e4129fd) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_32de9e954f ~p ~op ~loc ~style ~explode (_x : t_32de9e954f) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("account", begin match _x.account with | None -> `Null | Some _x ->
        `String end);
       ("customer", begin match _x.customer with | None -> `Null | Some _x ->
        `String end)]) ~ctr:(Json_encoding.construct Encoders'.t_32de9e954f)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_32de9e954f ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_32de9e954f) in _string_to ~p
    ~kind:(`ObjectN [("", `Any); ("account", `String); ("customer", `String)])
    ~dtr ~loc ~style ~explode _x
...
let string_of_t_656d49aac0 ~p ~op ~loc ~style ~explode (_x : t_656d49aac0) =
  _string_of ~kind:(
    begin match _x with
    | T_3a94416575 _x ->
      `ObjectN
...
let string_of_t_83d5590180 ~p ~op ~loc ~style ~explode (_x : t_83d5590180) =
  _string_of ~kind:(
...
let string_to_t_83d5590180 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_83d5590180) in _string_to ~p
...
let string_of_t_a63e6bd7de ~p ~op ~loc ~style ~explode (_x : t_a63e6bd7de) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("gt", begin match _x.gt with | None -> `Null | Some _x -> `Integer
        end);
       ("gte", begin match _x.gte with | None -> `Null | Some _x -> `Integer
        end);
       ("lt", begin match _x.lt with | None -> `Null | Some _x -> `Integer
        end);
       ("lte", begin match _x.lte with | None -> `Null | Some _x -> `Integer
        end)]) ~ctr:(Json_encoding.construct Encoders'.t_a63e6bd7de)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_a63e6bd7de ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_a63e6bd7de) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any); ("gt", `Integer); ("gte", `Integer);
              ("lt", `Integer); ("lte", `Integer)]) ~dtr ~loc ~style ~explode
    _x
...
let string_of_t_b566f1b6bc ~p ~op ~loc ~style ~explode (_x : t_b566f1b6bc) =
  _string_of ~kind:(
    begin match _x with
    | T_862da605ac _x ->
      `ObjectN
...
let string_of_t_53be0c51ff ~p ~op ~loc ~style ~explode (_x : t_53be0c51ff) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_53be0c51ff)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_53be0c51ff ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_53be0c51ff) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_8333ac0d50 ~p ~op ~loc ~style ~explode (_x : t_8333ac0d50) =
  _string_of ~kind:(
    begin match _x with
    | T_da2453d199 _x ->
      `ObjectN
...
let string_of_t_060ea1562f ~p ~op ~loc ~style ~explode (_x : t_060ea1562f) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_060ea1562f)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_060ea1562f ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_060ea1562f) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_7181394bbb ~p ~op ~loc ~style ~explode (_x : t_7181394bbb) =
  _string_of ~kind:(
    begin match _x with
    | T_6c4276e1a4 _x ->
      `ObjectN
...
let string_of_t_3d308e0087 ~p ~op ~loc ~style ~explode (_x : t_3d308e0087) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_3d308e0087)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_3d308e0087 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_3d308e0087) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_5fae893ff7 ~p ~op ~loc ~style ~explode (_x : t_5fae893ff7) =
  _string_of ~kind:(
    begin match _x with
    | T_eb52c2c010 _x ->
      `ObjectN
...
let string_of_t_e8ff8d2aa0 ~p ~op ~loc ~style ~explode (_x : t_e8ff8d2aa0) =
  _string_of ~kind:(
    begin match _x with
    | T_4c63ae17d6 _x ->
      `ObjectN
...
let string_of_t_2e0259263b ~p ~op ~loc ~style ~explode (_x : t_2e0259263b) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_2e0259263b)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_2e0259263b ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_2e0259263b) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_a51466ec35 ~p ~op ~loc ~style ~explode (_x : t_a51466ec35) =
  _string_of ~kind:(
    `ObjectN
...
let string_to_t_a51466ec35 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_a51466ec35) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any); ("enabled", `Boolean);
              ("liability",
               `ObjectN [("", `Any); ("account", `String); ("type", `String)])])
    ~dtr ~loc ~style ~explode _x
...
let string_of_t_50aa3198c9 ~p ~op ~loc ~style ~explode (_x : t_50aa3198c9) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_50aa3198c9)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_50aa3198c9 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_50aa3198c9) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_c4767cb749 ~p ~op ~loc ~style ~explode (_x : t_c4767cb749) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
                ...
                (`ObjectN
                   [("", `Any);
                    ...
                    ("billing_cycle_anchor",
                     begin match _x.billing_cycle_anchor with | None -> `Null
                     | Some _x -> `String end);
                    ("billing_thresholds",
                    ...
                    ("end_date", begin match _x.end_date with | None -> `Null
                     | Some _x ->
                     begin match _x with
                     | Ptime_t _x -> `Null
                     | T_2fa9ab7487 _x -> `Null
                     end end);
                    ("invoice_settings", begin match _x.invoice_settings with
                     | None -> `Null | Some _x ->
                     `ObjectN
                       [("", `Null);
                        ("account_tax_ids",
                         begin match _x.account_tax_ids with | None -> `Null
                         | Some _x -> `Null end);
                        ("days_until_due", begin match _x.days_until_due with
                         | None -> `Null | Some _x -> `Null end);
                        ("issuer", begin match _x.issuer with | None -> `Null
                         | Some _x -> `Null end)]
                     end);
                    ...
                    ("transfer_data", begin match _x.transfer_data with
                     ...
                     `ObjectN
                       [("", `Null);
                        ("amount_percent", begin match _x.amount_percent with
                         | None -> `Null | Some _x -> `Null end);
...
let string_to_t_c4767cb749 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_c4767cb749) in _string_to ~p
    ~kind:(`ObjectN
                 ...
                 [(`List
                     (`ObjectN
                        [("", `Any);
                         ...
                         ("trial_end", `Choice
                                       [`Null; `Null])]))]);
              ("proration_behavior", `String)]) ~dtr ~loc ~style ~explode _x
...
let string_of_t_312ad6306f ~p ~op ~loc ~style ~explode (_x : t_312ad6306f) =
  _string_of ~kind:(
    begin match _x with
...
let string_of_t_b9ba448b2f ~p ~op ~loc ~style ~explode (_x : t_b9ba448b2f) =
  _string_of ~kind:(
    begin match _x with
    | Ptime_t _x -> `Integer
...
let string_of_t_0178e1face ~p ~op ~loc ~style ~explode (_x : t_0178e1face) =
  _string_of ~kind:(
    begin match _x with
    | StringList _x ->
      `Array ((List.map (fun (_x : string) -> `Singleton (`String)) _x))
...
let string_of_t_baccbfb036 ~p ~op ~loc ~style ~explode (_x : t_baccbfb036) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ...
       ("proration_behavior", begin match _x.proration_behavior with
        | None -> `Null | Some _x -> `String end);
       ("proration_date", begin match _x.proration_date with | None -> `Null
        | Some _x -> `Integer end);
...
let string_to_t_baccbfb036 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_baccbfb036) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any);
                         ...
                         ("price_data",
                          `ObjectN
                            [("", `Null); ("currency", `Null);
                             ...
                             ("tax_behavior", `Null); ("unit_amount", `Null);
                             ("unit_amount_decimal", `Null)]);
                         ("quantity", `Integer);
                         ("tax_rates", `Choice
                                       [`Null; `Null])]))]);
              ("proration_behavior", `String); ("proration_date", `Integer);
              ("resume_at", `String); ("start_date", `Integer);
              ("trial_end", `Choice
                            [`String; `Integer])]) ~dtr ~loc ~style ~explode
    _x
...
let string_of_t_671de99c34 ~p ~op ~loc ~style ~explode (_x : t_671de99c34) =
  _string_of ~kind:(
    `Array
      ((List.map (fun (_x : t_5935fcbdc2) ->
          `Singleton
            (`ObjectN
               [("", `Any);
                ("billing_thresholds", begin match _x.billing_thresholds with
                 | None -> `Null | Some _x ->
                 begin match _x with
                 | T_bb54fadb7d _x ->
                   `ObjectN
                ...
                ("price", begin match _x.price with | None -> `Null
                 | Some _x -> `String end);
                ("price_data", begin match _x.price_data with | None -> `Null
                 | Some _x ->
                 `ObjectN
                   [("", `Any);
                    ...
                    ("recurring", let _x = _x.recurring in
                     `ObjectN
                       [("", `Null);
                        ...
                        ("interval_count", begin match _x.interval_count with
                         | None -> `Null | Some _x -> `Null end)]);
                    ("tax_behavior", begin match _x.tax_behavior with
                     | None -> `Null | Some _x -> `String end);
                    ("unit_amount", begin match _x.unit_amount with
...
let string_to_t_671de99c34 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_671de99c34) in _string_to ~p
    ~kind:(`Array
             [(`List
                 (`ObjectN
                    [("", `Any);
                     ("billing_thresholds",
                      `Choice
                      [`ObjectN [("", `Null); ("usage_gte", `Null)]; `String]);
                     ("clear_usage", `Boolean); ("deleted", `Boolean);
                     ("discounts",
                      `Choice
                      [`Array [(`List (`Null))]; `String]);
...
let string_of_t_6dff880c24 ~p ~op ~loc ~style ~explode (_x : t_6dff880c24) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_6dff880c24)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_6dff880c24 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_6dff880c24) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_50e9f09abe ~p ~op ~loc ~style ~explode (_x : t_50e9f09abe) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_50e9f09abe)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_50e9f09abe ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_50e9f09abe) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_30748e2d12 ~p ~op ~loc ~style ~explode (_x : t_30748e2d12) =
  _string_of ~kind:(
    begin match _x with
...
let string_of_t_c87881fc5c ~p ~op ~loc ~style ~explode (_x : t_c87881fc5c) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
          ...
          `ObjectN
            [("", `Any);
             ...
             ("country", begin match _x.country with | None -> `Null
              | Some _x -> `String end);
             ("line1", begin match _x.line1 with | None -> `Null | Some _x ->
              `String end);
             ("line2", begin match _x.line2 with | None -> `Null | Some _x ->
              `String end);
             ("postal_code", begin match _x.postal_code with | None -> `Null
              | Some _x -> `String end);
             ("state", begin match _x.state with | None -> `Null | Some _x ->
              `String end)]
        | T_24529d85b7 _x -> `String
        end end);
       ("shipping", begin match _x.shipping with | None -> `Null | Some _x ->
        begin match _x with
        | T_d2075e2bfe _x ->
          `ObjectN
            [("", `Any);
             ...
             ("phone", begin match _x.phone with | None -> `Null | Some _x ->
              `String end)]
...
let string_to_t_c87881fc5c ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_c87881fc5c) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any);
              ("address",
               `Choice
...
let string_of_t_873409613d ~p ~op ~loc ~style ~explode (_x : t_873409613d) =
  _string_of ~kind:(
    begin match _x with
    | T_31c19e5e08 _x ->
      `Array
...
let string_to_t_873409613d ~p ~loc ~style ~explode (_x : string) =
  [(let kind = `Array
                 [(`List
                     (`ObjectN
                        [("", `Any); ("coupon", `String);
                         ("discount", `String); ("promotion_code", `String)]))] in
...
let string_of_t_96382fbbc2 ~p ~op ~loc ~style ~explode (_x : t_96382fbbc2) =
  _string_of ~kind:(
    `Array
...
let string_to_t_96382fbbc2 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_96382fbbc2) in _string_to ~p
    ~kind:(`Array
             [(`List
                 (`ObjectN
                    [("", `Any); ("amount", `Integer); ("currency", `String);
...
let string_of_t_266682ce3a ~p ~op ~loc ~style ~explode (_x : t_266682ce3a) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("account", begin match _x.account with | None -> `Null | Some _x ->
...
let string_to_t_266682ce3a ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_266682ce3a) in _string_to ~p
...
let string_of_t_9aa5bd9e73 ~p ~op ~loc ~style ~explode (_x : t_9aa5bd9e73) =
  _string_of ~kind:(
    begin match _x with
    | String_ _x -> `String
...
let string_of_t_25cc6e6754 ~p ~op ~loc ~style ~explode (_x : t_25cc6e6754) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any); ("enabled", let _x = _x.enabled in `Boolean);
       ("liability", begin match _x.liability with | None -> `Null
...
let string_to_t_25cc6e6754 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_25cc6e6754) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any); ("enabled", `Boolean);
              ("liability",
               `ObjectN [("", `Any); ("account", `String); ("type", `String)])])
...
let string_of_t_d3294049d8 ~p ~op ~loc ~style ~explode (_x : t_d3294049d8) =
  _string_of ~kind:(
    begin match _x with
    | String_ _x -> `String
...
let string_of_t_f90350482b ~p ~op ~loc ~style ~explode (_x : t_f90350482b) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_f90350482b)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_f90350482b ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_f90350482b) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_4592f6749b ~p ~op ~loc ~style ~explode (_x : t_4592f6749b) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("end_behavior", begin match _x.end_behavior with | None -> `Null
...
let string_to_t_4592f6749b ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_4592f6749b) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any); ("end_behavior", `String);
              ("phases",
               `Array
...
let string_of_t_ed69666899 ~p ~op ~loc ~style ~explode (_x : t_ed69666899) =
  _string_of ~kind:(
    begin match _x with
...
let string_of_t_1d44572454 ~p ~op ~loc ~style ~explode (_x : t_1d44572454) =
  _string_of ~kind:(
    begin match _x with
    | Ptime_t _x -> `Integer
...
let string_of_t_8f5fa696e4 ~p ~op ~loc ~style ~explode (_x : t_8f5fa696e4) =
  _string_of ~kind:(
    begin match _x with
    | StringList _x ->
      `Array ((List.map (fun (_x : string) -> `Singleton (`String)) _x))
...
let string_of_t_7cca4f7dbf ~p ~op ~loc ~style ~explode (_x : t_7cca4f7dbf) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("billing_cycle_anchor", begin match _x.billing_cycle_anchor with
...
let string_to_t_7cca4f7dbf ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_7cca4f7dbf) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any);
              ("billing_cycle_anchor", `Choice
                                       [`String; `Integer]);
...
let string_of_t_1d8dcb799a ~p ~op ~loc ~style ~explode (_x : t_1d8dcb799a) =
  _string_of ~kind:(
    `Array
...
let string_to_t_1d8dcb799a ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_1d8dcb799a) in _string_to ~p
    ~kind:(`Array
             [(`List
                 (`ObjectN
                    [("", `Any);
...
let string_of_t_b3cdff625a ~p ~op ~loc ~style ~explode (_x : t_b3cdff625a) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_b3cdff625a)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_b3cdff625a ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_b3cdff625a) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_f0d6f3faa7 ~p ~op ~loc ~style ~explode (_x : t_f0d6f3faa7) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_f0d6f3faa7)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_f0d6f3faa7 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_f0d6f3faa7) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_8a9e68982d ~p ~op ~loc ~style ~explode (_x : t_8a9e68982d) =
  _string_of ~kind:(
    begin match _x with
...
let string_of_t_428991b112 ~p ~op ~loc ~style ~explode (_x : t_428991b112) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("address", begin match _x.address with | None -> `Null | Some _x ->
...
let string_to_t_428991b112 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_428991b112) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any);
              ("address",
               `Choice
...
let string_of_t_ec12d0adc5 ~p ~op ~loc ~style ~explode (_x : t_ec12d0adc5) =
  _string_of ~kind:(
    begin match _x with
    | T_ce45932982 _x ->
      `Array
...
let string_of_t_6de1e50279 ~p ~op ~loc ~style ~explode (_x : t_6de1e50279) =
  _string_of ~kind:(
    `Array
...
let string_to_t_6de1e50279 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_6de1e50279) in _string_to ~p
    ~kind:(`Array
             [(`List
                 (`ObjectN
                    [("", `Any); ("amount", `Integer); ("currency", `String);
...
let string_of_t_f2803c8215 ~p ~op ~loc ~style ~explode (_x : t_f2803c8215) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("account", begin match _x.account with | None -> `Null | Some _x ->
...
let string_to_t_f2803c8215 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_f2803c8215) in _string_to ~p
    ~kind:(`ObjectN [("", `Any); ("account", `String); ("type", `String)])
    ~dtr ~loc ~style ~explode _x
...
let string_of_t_28b874eafa ~p ~op ~loc ~style ~explode (_x : t_28b874eafa) =
  _string_of ~kind:(
    begin match _x with
    | T_aa2791e997 _x ->
      `ObjectN
...
let string_of_t_e7c047a717 ~p ~op ~loc ~style ~explode (_x : t_e7c047a717) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_e7c047a717)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_e7c047a717 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_e7c047a717) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_5e23dd8315 ~p ~op ~loc ~style ~explode (_x : t_5e23dd8315) =
  _string_of ~kind:(
    begin match _x with
    | T_6da4b6eaaf _x ->
      `ObjectN
...
let string_of_t_d05018810d ~p ~op ~loc ~style ~explode (_x : t_d05018810d) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_d05018810d)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_d05018810d ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_d05018810d) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_9b139b4e87 ~p ~op ~loc ~style ~explode (_x : t_9b139b4e87) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_9b139b4e87)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_9b139b4e87 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_9b139b4e87) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_fd32e6450f ~p ~op ~loc ~style ~explode (_x : t_fd32e6450f) =
  _string_of ~kind:(
    begin match _x with
    | T_c2905ff31f _x ->
      `ObjectN
...
let string_of_t_e2b1b56eac ~p ~op ~loc ~style ~explode (_x : t_e2b1b56eac) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_e2b1b56eac)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_e2b1b56eac ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_e2b1b56eac) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_e42a3414c1 ~p ~op ~loc ~style ~explode (_x : t_e42a3414c1) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_e42a3414c1)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_e42a3414c1 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_e42a3414c1) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_be86c12b19 ~p ~op ~loc ~style ~explode (_x : t_be86c12b19) =
  _string_of ~kind:(
    begin match _x with
    | T_acdb658c9d _x ->
      `ObjectN
...
let string_of_t_4b1fd7d313 ~p ~op ~loc ~style ~explode (_x : t_4b1fd7d313) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_4b1fd7d313)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_4b1fd7d313 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_4b1fd7d313) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_6565ec2878 ~p ~op ~loc ~style ~explode (_x : t_6565ec2878) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
...
let string_to_t_6565ec2878 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_6565ec2878) in _string_to ~p
    ~kind:(`ObjectN
...
let string_of_t_386a8a09fc ~p ~op ~loc ~style ~explode (_x : t_386a8a09fc) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_386a8a09fc)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_386a8a09fc ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_386a8a09fc) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_3d82a38285 ~p ~op ~loc ~style ~explode (_x : t_3d82a38285) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_3d82a38285)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_3d82a38285 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_3d82a38285) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_00d4956c80 ~p ~op ~loc ~style ~explode (_x : t_00d4956c80) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_00d4956c80)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_00d4956c80 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_00d4956c80) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_a985a64df2 ~p ~op ~loc ~style ~explode (_x : t_a985a64df2) =
  _string_of ~kind:(
    begin match _x with
    | T_b6181859f2 _x ->
      `ObjectN
...
let string_of_t_9463e4703f ~p ~op ~loc ~style ~explode (_x : t_9463e4703f) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_9463e4703f)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_9463e4703f ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_9463e4703f) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_a6ed41322f ~p ~op ~loc ~style ~explode (_x : t_a6ed41322f) =
  _string_of ~kind:(
    begin match _x with
    | T_0bcc6214c1 _x ->
      `ObjectN
...
let string_of_t_a13fab6ce7 ~p ~op ~loc ~style ~explode (_x : t_a13fab6ce7) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_a13fab6ce7)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_a13fab6ce7 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_a13fab6ce7) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_89676accde ~p ~op ~loc ~style ~explode (_x : t_89676accde) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("account", begin match _x.account with | None -> `Null | Some _x ->
        `String end);
       ("customer", begin match _x.customer with | None -> `Null | Some _x ->
        `String end)]) ~ctr:(Json_encoding.construct Encoders'.t_89676accde)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_89676accde ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_89676accde) in _string_to ~p
    ~kind:(`ObjectN [("", `Any); ("account", `String); ("customer", `String)])
    ~dtr ~loc ~style ~explode _x
...
let string_of_t_94e066c621 ~p ~op ~loc ~style ~explode (_x : t_94e066c621) =
  _string_of ~kind:(
    begin match _x with
    | T_92de2c55be _x ->
      `ObjectN
...
let string_of_t_41b2207b76 ~p ~op ~loc ~style ~explode (_x : t_41b2207b76) =
  _string_of ~kind:(
    begin match _x with
    | String_ _x -> `String
...
let string_of_t_735ee27343 ~p ~op ~loc ~style ~explode (_x : t_735ee27343) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_735ee27343)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_735ee27343 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_735ee27343) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_dfda23638e ~p ~op ~loc ~style ~explode (_x : t_dfda23638e) =
  _string_of ~kind:(
    begin match _x with
    | T_f942927e0c _x ->
      `ObjectN
...
let string_of_t_78f6837d46 ~p ~op ~loc ~style ~explode (_x : t_78f6837d46) =
  _string_of ~kind:(
    begin match _x with
    | T_fb7e928310 _x ->
      `ObjectN
...
let string_of_t_c85fb118c6 ~p ~op ~loc ~style ~explode (_x : t_c85fb118c6) =
  _string_of ~kind:(
    begin match _x with
    | T_918279e85e _x ->
      `ObjectN
...
let string_of_t_5c423aae2d ~p ~op ~loc ~style ~explode (_x : t_5c423aae2d) =
  _string_of ~kind:(
    begin match _x with
    | T_156f1957fc _x ->
      `ObjectN
...
let string_of_t_806c738ca8 ~p ~op ~loc ~style ~explode (_x : t_806c738ca8) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_806c738ca8)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_806c738ca8 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_806c738ca8) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_54d3503cdb ~p ~op ~loc ~style ~explode (_x : t_54d3503cdb) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("interval", begin match _x.interval with | None -> `Null | Some _x ->
        `String end);
...
let string_to_t_54d3503cdb ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_54d3503cdb) in _string_to ~p
    ~kind:(`ObjectN
...
let string_of_t_c04b129744 ~p ~op ~loc ~style ~explode (_x : t_c04b129744) =
  _string_of ~kind:(
    begin match _x with
    | T_818f6dfe63 _x ->
      `ObjectN
...
let string_of_t_1d3358b59f ~p ~op ~loc ~style ~explode (_x : t_1d3358b59f) =
  _string_of ~kind:(
    begin match _x with
    | T_5a93111bd9 _x ->
      `ObjectN
...
let string_of_t_b2c88d22a6 ~p ~op ~loc ~style ~explode (_x : t_b2c88d22a6) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_b2c88d22a6)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_b2c88d22a6 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_b2c88d22a6) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_902cd52d55 ~p ~op ~loc ~style ~explode (_x : t_902cd52d55) =
  _string_of ~kind:(
    begin match _x with
    | T_0942bf866d _x ->
      `ObjectN
...
let string_of_t_438f8e3e47 ~p ~op ~loc ~style ~explode (_x : t_438f8e3e47) =
  _string_of ~kind:(
    begin match _x with
    | T_6c02505772 _x ->
      `ObjectN
...
let string_of_t_b205fb898b ~p ~op ~loc ~style ~explode (_x : t_b205fb898b) =
  _string_of ~kind:(
    begin match _x with
    | T_f99ca180bb _x ->
      `ObjectN
...
let string_of_t_c862988285 ~p ~op ~loc ~style ~explode (_x : t_c862988285) =
  _string_of ~kind:(
    begin match _x with
    | T_a7f5f95f2b _x ->
      `ObjectN
...
let string_of_t_95b7b73ff8 ~p ~op ~loc ~style ~explode (_x : t_95b7b73ff8) =
  _string_of ~kind:(
    begin match _x with
    | T_3e044be4e2 _x ->
      `ObjectN
...
let string_of_t_91991d11ce ~p ~op ~loc ~style ~explode (_x : t_91991d11ce) =
  _string_of ~kind:(
    begin match _x with
    | T_d267e559df _x ->
      `ObjectN
...
let string_of_t_62ebe57aa0 ~p ~op ~loc ~style ~explode (_x : t_62ebe57aa0) =
  _string_of ~kind:(
    begin match _x with
    | T_c8869a1e90 _x ->
      `ObjectN
...
let string_of_t_47dcc46a23 ~p ~op ~loc ~style ~explode (_x : t_47dcc46a23) =
  _string_of ~kind:(
    begin match _x with
    | T_0a007f4c7b _x ->
      `ObjectN
...
let string_of_t_3115fde6ed ~p ~op ~loc ~style ~explode (_x : t_3115fde6ed) =
  _string_of ~kind:(
    begin match _x with
    | T_8e372f4d99 _x ->
      `ObjectN
...
let string_of_t_edb9c04aba ~p ~op ~loc ~style ~explode (_x : t_edb9c04aba) =
  _string_of ~kind:(
    begin match _x with
    | T_8dfd931249 _x ->
      `ObjectN
...
let string_of_t_b053f4a10e ~p ~op ~loc ~style ~explode (_x : t_b053f4a10e) =
  _string_of ~kind:(
    begin match _x with
    | T_7e5129cd97 _x ->
      `ObjectN
...
let string_of_t_8dd946adeb ~p ~op ~loc ~style ~explode (_x : t_8dd946adeb) =
  _string_of ~kind:(
    begin match _x with
    | T_704964c184 _x ->
      `ObjectN
...
let string_of_t_e65a202262 ~p ~op ~loc ~style ~explode (_x : t_e65a202262) =
  _string_of ~kind:(
    begin match _x with
    | T_153437e623 _x ->
      `ObjectN
...
let string_of_t_b1188f80a4 ~p ~op ~loc ~style ~explode (_x : t_b1188f80a4) =
  _string_of ~kind:(
...
let string_to_t_b1188f80a4 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_b1188f80a4) in _string_to ~p
...
let string_of_t_d2098cac25 ~p ~op ~loc ~style ~explode (_x : t_d2098cac25) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_d2098cac25)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_d2098cac25 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_d2098cac25) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_a6b80cd539 ~p ~op ~loc ~style ~explode (_x : t_a6b80cd539) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_a6b80cd539)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_a6b80cd539 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_a6b80cd539) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_900f55e0e4 ~p ~op ~loc ~style ~explode (_x : t_900f55e0e4) =
  _string_of ~kind:(
    begin match _x with
    | T_c53ac2a9cd _x ->
      `ObjectN
...
let string_of_t_dad4a4ac5b ~p ~op ~loc ~style ~explode (_x : t_dad4a4ac5b) =
  _string_of ~kind:(
    begin match _x with
    | T_d1eb4a98f5 _x ->
      `ObjectN
...
let string_of_t_dce85b0bb2 ~p ~op ~loc ~style ~explode (_x : t_dce85b0bb2) =
  _string_of ~kind:(
    begin match _x with
    | T_f89b48aae9 _x ->
      `ObjectN
...
let string_of_t_66eb6371e3 ~p ~op ~loc ~style ~explode (_x : t_66eb6371e3) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_66eb6371e3)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_66eb6371e3 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_66eb6371e3) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_17690afaaf ~p ~op ~loc ~style ~explode (_x : t_17690afaaf) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
       ("account", begin match _x.account with | None -> `Null | Some _x ->
...
let string_to_t_17690afaaf ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_17690afaaf) in _string_to ~p
    ~kind:(`ObjectN
...
let string_of_t_ef1df523db ~p ~op ~loc ~style ~explode (_x : t_ef1df523db) =
  _string_of ~kind:(
    begin match _x with
    | T_81c4215133 _x ->
      `ObjectN
...
let string_of_t_477c159e47 ~p ~op ~loc ~style ~explode (_x : t_477c159e47) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_477c159e47)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_477c159e47 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_477c159e47) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_76a6cf8994 ~p ~op ~loc ~style ~explode (_x : t_76a6cf8994) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_76a6cf8994)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_76a6cf8994 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_76a6cf8994) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_6684cf5aa7 ~p ~op ~loc ~style ~explode (_x : t_6684cf5aa7) =
  _string_of ~kind:(
    begin match _x with
    | T_1149c5a72c _x ->
      `ObjectN
...
let string_of_t_ca787dca43 ~p ~op ~loc ~style ~explode (_x : t_ca787dca43) =
  _string_of ~kind:(
    begin match _x with
    | T_5a45966f13 _x ->
      `ObjectN
...
let string_of_t_ce1d711154 ~p ~op ~loc ~style ~explode (_x : t_ce1d711154) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_ce1d711154)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_ce1d711154 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_ce1d711154) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_c5c22bba1c ~p ~op ~loc ~style ~explode (_x : t_c5c22bba1c) =
  _string_of ~kind:(
    begin match _x with
    | T_5b06d5fa4f _x ->
      `ObjectN
...
let string_of_t_de06274f51 ~p ~op ~loc ~style ~explode (_x : t_de06274f51) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_de06274f51)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_de06274f51 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_de06274f51) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_ed1d428d1e ~p ~op ~loc ~style ~explode (_x : t_ed1d428d1e) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_ed1d428d1e)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_ed1d428d1e ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_ed1d428d1e) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_e27a5a13e2 ~p ~op ~loc ~style ~explode (_x : t_e27a5a13e2) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_e27a5a13e2)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_e27a5a13e2 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_e27a5a13e2) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_9096085c5b ~p ~op ~loc ~style ~explode (_x : t_9096085c5b) =
  _string_of ~kind:(
    begin match _x with
    | T_8bc900449d _x ->
      `ObjectN
...
let string_of_t_bda61eb198 ~p ~op ~loc ~style ~explode (_x : t_bda61eb198) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_bda61eb198)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_bda61eb198 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_bda61eb198) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_fd0b444db8 ~p ~op ~loc ~style ~explode (_x : t_fd0b444db8) =
  _string_of ~kind:(
    begin match _x with
    | T_34d236a9e5 _x ->
      `ObjectN
...
let string_of_t_e134f021a2 ~p ~op ~loc ~style ~explode (_x : t_e134f021a2) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_e134f021a2)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_e134f021a2 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_e134f021a2) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_7e023d3347 ~p ~op ~loc ~style ~explode (_x : t_7e023d3347) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_7e023d3347)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_7e023d3347 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_7e023d3347) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_99ae8c4576 ~p ~op ~loc ~style ~explode (_x : t_99ae8c4576) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
...
let string_to_t_99ae8c4576 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_99ae8c4576) in _string_to ~p
...
let string_of_t_ae96c50be4 ~p ~op ~loc ~style ~explode (_x : t_ae96c50be4) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_ae96c50be4)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_ae96c50be4 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_ae96c50be4) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_fe86a832fb ~p ~op ~loc ~style ~explode (_x : t_fe86a832fb) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_fe86a832fb)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_fe86a832fb ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_fe86a832fb) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_01243047ea ~p ~op ~loc ~style ~explode (_x : t_01243047ea) =
  _string_of ~kind:(
    begin match _x with
    | T_8a5740cf7a _x ->
      `ObjectN
...
let string_of_t_fa33b5a2bd ~p ~op ~loc ~style ~explode (_x : t_fa33b5a2bd) =
  _string_of ~kind:(
    begin match _x with
    | T_99d78357ad _x ->
      `ObjectN
...
let string_of_t_1f08681071 ~p ~op ~loc ~style ~explode (_x : t_1f08681071) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_1f08681071)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_1f08681071 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_1f08681071) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_2bb01eccbd ~p ~op ~loc ~style ~explode (_x : t_2bb01eccbd) =
  _string_of ~kind:(
    begin match _x with
    | T_f34b1e7619 _x ->
      `ObjectN
...
let string_of_t_65e58eb6da ~p ~op ~loc ~style ~explode (_x : t_65e58eb6da) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_65e58eb6da)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_65e58eb6da ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_65e58eb6da) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_6f04380d09 ~p ~op ~loc ~style ~explode (_x : t_6f04380d09) =
  _string_of ~kind:( `String)
    ~ctr:(Json_encoding.construct Encoders'.t_6f04380d09)
    ~p ~op ~loc ~style ~explode _x

let string_to_t_6f04380d09 ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_6f04380d09) in _string_to ~p
    ~kind:(`String) ~dtr ~loc ~style ~explode _x
...
let string_of_t_b458f1b6fb ~p ~op ~loc ~style ~explode (_x : t_b458f1b6fb) =
  _string_of ~kind:(
    `ObjectN
      [("", `Any);
...
let string_to_t_b458f1b6fb ~p ~loc ~style ~explode (_x : string) =
  let dtr = (Json_encoding.destruct Encoders'.t_b458f1b6fb) in _string_to ~p
    ~kind:(`ObjectN
             [("", `Any);
              ("posted_at",
               `Choice
